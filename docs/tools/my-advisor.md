...
2. Gather related past decisions: reuse MyKnowledger's shortlist logic
   (keyword/date match over the merged ledger) — this makes MyAdvisor a
   consumer of MyKnowledger's approach, see open question below on how
...
5. **Do not skip the Engine call on an empty bundle** — unlike MyKnowledger,
...
- **Reads:** same merged ledger + dev-ledger stream as MyKnowledger, plus a
...
- **Edge case (empty context bundle):** no related decisions or files
  found; assert the Engine is still called (unlike MyKnowledger) with a
  prompt noting the absence, and the run still succeeds.
- Mock `github.Runner`; the context-assembly reuse of MyKnowledger/
  MySearcher logic is exercised against real temp fixtures, not mocked.

## Dependencies & build order

Depends on core `ledger`, `github`, `policy`, plus MyKnowledger's and
...
- **Cross-tool reuse mechanism isn't settled.** MyAdvisor wanting
  MyKnowledger's and MySearcher's shortlist logic raises a real question:
  ...
  duplication is confirmed (i.e. after MyKnowledger and MySearcher both
